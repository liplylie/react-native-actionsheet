...
class ActionSheet extends React.Component {
  ...
  _createButton (title, index) {
    ...
    const cancelButtonStyle = cancelButtonIndex === index ? this.props.styles.cancelButtonStyle : {}
    return (
      <TouchableHighlight
        key={index}
        activeOpacity={1}
      ...
      >
        {React.isValidElement(title) ? title : (
          <Text style={[styles.buttonText, {color: fontColor}, cancelButtonStyle]}>{title}</Text>
        )}
      </TouchableHighlight>
    )
  }
      ...
      >
        <View style={[styles.wrapper]}>
          ...
          >
            {this._renderTitle()}
            {this._renderMessage()}
            <ScrollView scrollEnabled={this.scrollEnabled} style={this.props.styles.scrollView}>{this._renderOptions()}</ScrollView>
